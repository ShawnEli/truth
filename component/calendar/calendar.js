...
Component({
    ...
    methods: {
        ...
        selectday(e) {
            let thischoose
            if (e) {
                thischoose = e.currentTarget.dataset.index + "|" + e.currentTarget.dataset.indexs
            }
            let choose = this.data.preChoose.split("|")
            let date = this.data.date

            for (let i = 0; i < date.length; i++) {
                let len = date[i].length
                for (let j = 0; j < len; j++) {
                    if(i == choose[0] && j == choose[1]){
                        date[i][j].selected = 1
                    } else {
                        date[i][j].selected = 0
                    }
            ...
            console.log(this.data.date[choose[0]][choose[1]].re)
            this.setData({
